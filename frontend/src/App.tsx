...
function App() {
	...
	const deviceContext = useMemo(
		() => toDeviceContext(currentSession?.analysis ?? null),
		[currentSession],
	);
	...
	async function handleAnalyze() {
		...
		try {
			...
			updateSessionAnalysis(currentSessionId, result);
		} catch (requestError) {
			setError(
	...
	async function handleExplain() {
		if (!selectedFile || !deviceContext || !currentSessionId) {
	...
	async function handleGenerateInstructions() {
		...
		try {
			...
			setInstructions(currentSessionId, steps);
		} catch (requestError) {
			setError(
				requestError instanceof Error
					? requestError.message
		...
		<div className="min-h-screen bg-stone-100 text-stone-900 transition-colors dark:bg-stone-950 dark:text-stone-100">
			<div className="mx-auto flex min-h-screen max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
				...
				<div className="grid gap-6 lg:grid-cols-[1fr_280px]">
					<main className="grid gap-6 lg:grid-cols-1">
									...
									{currentSession?.analysis ? (
										<div className="rounded-[1.25rem] border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950 dark:border-emerald-900/70 dark:bg-emerald-950/40 dark:text-emerald-100">
											...
											<div className="mt-3 text-lg font-semibold capitalize">
												{currentSession.analysis.device_type.replaceAll("_", " ")}
											</div>
											<div className="mt-2 text-sm">
												Уверенность: {formatConfidence(currentSession.analysis.confidence)}
											</div>
											<div className="mt-1 text-sm text-emerald-800 dark:text-emerald-200">
												Найдено элементов управления:{" "}
												{currentSession.analysis.detected_controls.length}
											</div>
										</div>
									) : null}
						...
						<section className="rounded-[2rem] border border-stone-200 bg-white p-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
							...
							<div className="mt-4 flex flex-wrap gap-2">
								{currentSession?.analysis?.suggested_categories.length ? (
									currentSession.analysis.suggested_categories.map((category) => (
										<span
											key={category}
							...
							<div className="mt-5 space-y-3">
								{currentSession?.analysis?.detected_controls.map((control) => (
									<div
										key={control.id}
						...
						<section className="rounded-[2rem] border border-stone-200 bg-white p-5 shadow-sm dark:border-stone-800 dark:bg-stone-900">
							...
							<div className="mt-4 space-y-3">
								{currentSession?.instructions.length ? (
									currentSession.instructions.map((step) => (
										<article
					...
					<aside className="rounded-[2rem] border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-800 dark:bg-stone-900">
						<h3 className="text-lg font-semibold">История сессий</h3>
									...
									>
										<div className="flex items-center justify-between">
											<span className="truncate">
												{session.analysis?.device_type.replaceAll("_", " ") ||
													"Новое изображение"}
											</span>
											<button
